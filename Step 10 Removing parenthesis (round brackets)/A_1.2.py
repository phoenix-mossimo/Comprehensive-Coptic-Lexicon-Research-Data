...
tree = etree.parse('XML/new_A_1.1.xml')
...
for e in entries:
    ...
    for f in forms:
        ...
        if  re.search ('^\(.*[^\)]$', form_text): # [^\)]$ means ")" is NOT at the end of the word
            n = n +1
            f1_copy = deepcopy(f)
            f2_copy = deepcopy(f)
...
#                etree.SubElement(f, "prt", type="substitutedBy", target="#CF_newform1")
#                etree.SubElement(f, "prt", type="substitutedBy", target="#CF_newform2")
...
tree.write("XML/new_A_1.2.xml", encoding="utf-8", pretty_print=True)
#outfile.close()