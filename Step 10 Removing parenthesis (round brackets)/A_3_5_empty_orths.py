...
# 1) check if there are empty <forms> and <orths>, if yes - print the ID
...
tree = etree.parse('XML/new_A_3_TMP_ID.xml')
...
for e in entries:
    ...
    for f in forms:                
        if len(xstr(f.text))  > 1:
            print (str((f.attrib).get('{http://www.w3.org/XML/1998/namespace}id', 'no')))            
        if  str(f.findtext("orth", "")) == "": 
            print (str((f.attrib).get('{http://www.w3.org/XML/1998/namespace}id', 'no')), len(f.findtext("orth", "")))
